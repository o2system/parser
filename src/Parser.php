...
use O2System\Parser\Registries\Config;
 ...
 */
class Parser extends AbstractRegistryPattern
{
    /**
     * Compiler Config
     *
     ...
     *
     * @param \O2System\Parser\Registries\Config $config
     */
    ...
    public function loadDriver ( $driverOffset )
    {
        ...
        if ( class_exists( $driverClassName ) ) {
            ...
            $this->register( $driverOffset, ( new $driverClassName() )->initialize( $config ) );
        }
    }

    public function addDriver ( Parser\Abstracts\AbstractDriver $driver, $driverOffset = null )
    {
        ...
        if ( $driver->isInitialize() ) {
            $this->register( $driverOffset, $driver );
        } else {
            $this->register( $driverOffset, $driver->initialize( $config ) );
        }
    }
